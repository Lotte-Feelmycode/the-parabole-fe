...
export default function IconList(){
  ...
  return (
    ...
    <div class="flex flex-col items-centermt-8 md:mt-16">
        ...
        {iconList.map((item) => (
          <div className="flex flex-col items-center">
            <div className="md:w-24 md:h-24 bg-gray-100 rounded-full overflow-hidden shadow-lg mb-2 md:mb-4">
              <img src={item.icon} loading="lazy" className="w-full h-full p-2 object-cover object-center" />
            </div>
            <p className="text-gray-500 text-sm md:text-base text-center mb-3 md:mb-4">{item.title}</p>
          </div>
        ))}
      </div>
    </div>
